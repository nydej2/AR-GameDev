...
namespace GoogleARCore.Examples.CloudAnchor
{
    ...
    public class CloudAnchorController : MonoBehaviour
    {
        ...
        private bool prefabInstatniated = false;

        private GameObject andyObject;
        private GameObject andyObjectInsta;
        ...
        public void Update()
        {
            ...
            if (Application.platform != RuntimePlatform.IPhonePlayer)
            {
                ...
                {
                    if(m_LastPlacedAnchor != null && prefabInstatniated) {
                        andyObject.GetComponent<MoveSofa>().StartMove(hit.Pose.position);
                    }

                    m_LastPlacedAnchor = hit.Trackable.CreateAnchor(hit.Pose);
                    Debug.Log("Neuer Anchor wurde erstellt: " + m_LastPlacedAnchor);
                }
            }
            //Nur Apple Zeugs!
            else
            ...
            if (m_LastPlacedAnchor != null && !prefabInstatniated)
            {
                ...
                /*Debug.Log("X: " + m_LastPlacedAnchor.transform.position.x);
                Debug.Log("Y: " + m_LastPlacedAnchor.transform.position.y);
                Debug.Log("Z: " + m_LastPlacedAnchor.transform.position.z);
                Vector3 pos = m_LastPlacedAnchor.transform.position;

                //pos.x = pos.x + 5.0f;
                pos.y = pos.y + 0.5f;
                //pos.z = pos.z + 5.0f;

                Debug.Log("X neu: " + pos.x);
                Debug.Log("Y neu: " + pos.y);
                Debug.Log("Z neu: " + pos.z);
                //m_LastPlacedAnchor.transform.position = pos;
                */

                //Instantiation at the location of the created Anchos Object.
                andyObject = Instantiate(_GetAndyPrefab(), m_LastPlacedAnchor.transform.position,
                    m_LastPlacedAnchor.transform.rotation);

                // Necessary rotation for model.
                andyObject.transform.Rotate(k_ModelRotation, 0, 0, Space.Self);

                // Make Andy model a child of the anchor.
                andyObject.transform.parent = m_LastPlacedAnchor.transform;

                prefabInstatniated = true;
            }
        ...
        public void OnEnterUploadModeClick()
        {
            if(m_CurrentMode == ApplicationMode.Hosting)
            {
